...
export function DeviceWizard({ onClose, onComplete }: DeviceWizardProps) {
  ...
  const detectFromADB = async () => {
    ...
    try {
      ...
      setUnregisteredSerials(devices.map((d: DeviceInfo) => d.serial));
    } catch {
      // ignore
    } finally {
          ...
          {step === 'serial' && (
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  {loadingSerials ? 'Scanning...' : 'Detect from ADB'}
                </button>
                {unregisteredSerials.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {unregisteredSerials.map((s) => (
                      <button
                        key={s}
                        onClick={() => setSerial(s)}
            ...
            <div className="space-y-4">
              ...
              {validationState === 'unauthorized' && (
                <div className="space-y-2">
                  ...
                  <div className="text-[10px] text-gray-500 bg-gray-800 rounded p-3">
                    Check the device screen for an &quot;Allow USB debugging?&quot; prompt and tap
                    &quot;Allow&quot;, then retry.
                  </div>
                </div>
              )}
              ...
              {validationError && (
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 text-xs bg-gray-800 hover:bg-gray-700 text-gray-300 rounded"
                  >
                    Retry
                  </button>
                </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs bg-brand-600 hover:bg-brand-700 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
            >
